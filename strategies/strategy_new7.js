...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  const sigma = 0.045;
  const subKeyBlend = 0.15; // blend sub-key toward parent
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      let prior = null;
      // Try fine key first
      const fineP = adaptiveModel[key] || allModel[key];
      const coarseP = adaptiveModel[baseKey] || allModel[baseKey];

      if (fineP && key !== baseKey && coarseP) {
        // Blend sub-key with parent for regularization
        prior = fineP.map((v, c) => (1 - subKeyBlend) * v + subKeyBlend * coarseP[c]);
      } else if (fineP) {
        prior = [...fineP];
      } else if (coarseP) {
        prior = [...coarseP];
        ...
        for (let len = baseKey.length - 1; len >= 1 && !prior; len--) {
