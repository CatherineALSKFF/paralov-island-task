...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  // --- Gaussian-weighted model (narrow sigma for sharp growth matching) ---
  ...
  // --- Top-K model (uniform over K closest) ---
  const K = config.K || 3;
  ...
  // --- Uniform model (minimal fallback) ---
  const uniWeights = {};
  ...
  function lookupWithShrinkage(model, key) {
    ...
    for (let trim = 1; trim < key.length; trim++) {
      ...
      if (!result) {
        ...
        const shrinkStrength = config.shrink || 3;
  ...
  // --- Predict ---
  const wGauss = config.wGauss || 0.75;
  const wTopK = config.wTopK || 0.24;
  const wUni = config.wUni || 0.01;
