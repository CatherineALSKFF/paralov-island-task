...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  const deathThreshold = config.DT || 0.05;
  const targetGrowth = growthRates[String(testRound)] || 0.15;
  const allRounds = Object.keys(perRoundBuckets).map(Number).filter(n => n !== testRound);
  const isDeath = targetGrowth < deathThreshold;

  // Split rounds by regime
  const deathRounds = allRounds.filter(r => (growthRates[String(r)] || 0.15) < deathThreshold);
  const growthRounds = allRounds.filter(r => (growthRates[String(r)] || 0.15) >= deathThreshold);

  // K-nearest baseline (from same regime if possible)
  ...
  // For death regime: simple average of death rounds (no regression needed)
  let deathModel = null;
  if (isDeath && deathRounds.length >= 2) {
    deathModel = mergeBuckets(perRoundBuckets, deathRounds);
  }

  // For growth regime: regression on growth rate
  const keyData = {};
  const coarseKeyData = {};
  const regRounds = isDeath ? allRounds : growthRounds;
  for (const r of regRounds) {
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      let basePrior;
      if (isDeath && deathModel) {
        basePrior = deathModel[key] || deathModel[ck] || adaptiveModel[key] || adaptiveModel[ck] || allModel[key] || allModel[ck] || null;
      } else {
        basePrior = adaptiveModel[key] || adaptiveModel[ck] || allModel[key] || allModel[ck] || null;
      }
      ...
      // For death rounds, rely more on K-nearest (less regression)
      const rb = isDeath ? Math.min(regBlend, 0.5) : regBlend;

      let prior;
      if (basePrior && regPrior) {
        prior = basePrior.map((v, c) => (1 - rb) * v + rb * regPrior[c]);
      } else if (regPrior) {
        prior = [...regPrior];
      } else if (basePrior) {
        prior = [...basePrior];
