...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  const regWeight = config.regWeight || 0.0;
  const linWeight = config.linWeight || 0.85;
  const ridge = config.ridge || 0.01; // ridge regularization for linear fit
  ...
  // Compute initial state features per round for multi-feature regression
  // For now just use growth rate as the feature (since we only have that)
  // But fit with ridge regularization for stability

  // Ridge-regularized linear model per key
  const linModel = {};
  for (const key of Object.keys(meanModel)) {
    ...
    for (let c = 0; c < 6; c++) {
      let sumG = 0, sumP = 0, sumGG = 0, sumGP = 0;
      const n = points.length;
      for (const pt of points) {
        sumG += pt.g; sumP += pt.dist[c];
        sumGG += pt.g * pt.g; sumGP += pt.g * pt.dist[c];
      }
      const meanG = sumG / n, meanP = sumP / n;
      // Ridge regression: b = sum((g-gbar)(p-pbar)) / (sum((g-gbar)^2) + ridge)
      let num = 0, den = 0;
      for (const pt of points) {
        num += (pt.g - meanG) * (pt.dist[c] - meanP);
        den += (pt.g - meanG) * (pt.g - meanG);
      }
      const b = num / (den + ridge);
      const a = meanP - b * meanG;
