...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  // Precompute nearest settlement distance for enriched keys
  ...
  // Enriched key: adds a/b suffixes for 1-settlement cells, n/m/f for isolated cells
  ...
  // Build augmented per-round bucket averages + derived 'd' sub-buckets
  var augBuckets = {};
  for (var rn of allRounds) {
    ...
    // Derive 'd' suffix (residual when a and b are subtracted from base)
  ...
  // LOESS: weighted linear regression p(class) = a + b * growth_rate
  var loessCache = {};
  ...
  // Gaussian-weighted mean model (1D: growth rate only)
  var roundWeights = {};
  var tw = 0;
  for (var rn of allRounds) {
  ...
  // Lookup: prefer enriched key, fall back to base key, blend LOESS + mean
  function lookupKey(fineKey, baseKey) {
    ...
    if (wmDist && lnDist) {
      return wmDist.map(function(v, c) { return (1 - linWeight) * v + linWeight * lnDist[c]; });
    }
    if (lnDist) return lnDist;
    if (wmDist) return wmDist;
    // Fallback chain: strip last character of base key
    var fb = baseKey;
    while (fb.length > 1) {
      fb = fb.slice(0, -1);
      if (meanModel[fb]) return meanModel[fb].slice();
    }
  ...
  for (var y = 0; y < H; y++) {
    ...
    for (var x = 0; x < W; x++) {
      ...
      // Adaptive floor: lower for confident (low entropy) cells
      var entropy = 0;
      for (var c = 0; c < 6; c++) {
