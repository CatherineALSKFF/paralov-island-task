...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  var sigma = config.sigma || 0.10;
  var floor = config.FLOOR || 0.00001;
  var linWeight = config.linWeight || 0.94;
  var loessSigma = config.loessSigma || 0.15;
  var loessReg = config.loessReg || 0; // blend fine LOESS toward coarse LOESS
  ...
  // LOESS regression (cached per key)
  var linCache = {};
  ...
  function lookupKey(fineKey, baseKey) {
    ...
    // Get LOESS at fine key level
    var lnFine = fitLoess(fineKey);
    // Get LOESS at base key level (coarser, more data)
    var lnBase = fineKey !== baseKey ? fitLoess(baseKey) : null;
    
    // Regularized LOESS: blend fine toward coarse
    var lnDist;
    if (lnFine && lnBase) {
      lnDist = lnFine.map(function(v, c) { return (1 - loessReg) * v + loessReg * lnBase[c]; });
    } else {
