...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  // Transform growth for regression (log-like, handles 0)
  const xform = g => Math.log(g + 0.005);
  const targetX = xform(targetGrowth);
  ...
  for (const r of allRounds) {
    ...
    for (const [key, val] of Object.entries(b)) {
      ...
      keyData[key].push({ x: xform(g), g, dist: val.sum.map(v => v / val.count) });
    }
    const roundCoarse = {};
    for (const [key, val] of Object.entries(b)) {
    ...
    for (const [ck, val] of Object.entries(roundCoarse)) {
      ...
      coarseKeyData[ck].push({ x: xform(g), g, dist: val.sum.map(v => v / val.count) });
    }
  }
  ...
  function fitRegression(data) {
    ...
    // Weight by proximity in ORIGINAL growth-rate space
    const ws = data.map(d => {
    ...
    let xMean = 0;
    for (let i = 0; i < data.length; i++) xMean += ws[i] * data[i].x;
    xMean /= wSum;
    ...
    for (let c = 0; c < 6; c++) {
      ...
      if (xVar < 1e-10) { result[c] = Math.max(0, pMean); continue; }

      let cov = 0;
      for (let i = 0; i < data.length; i++) {
        cov += ws[i] * (data[i].x - xMean) * (data[i].dist[c] - pMean);
      }
      cov /= wSum;

      result[c] = Math.max(0, pMean + (cov / xVar) * (targetX - xMean));
    }
