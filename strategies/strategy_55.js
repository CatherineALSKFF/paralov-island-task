...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  // Fit weighted local linear regression with R² for adaptive blend
  function fitRegression(data) {
    ...
    for (let c = 0; c < 6; c++) {
      ...
      for (let i = 0; i < data.length; i++) {
        const pred = pMean + slope * (data[i].g - gMean);
        const diff = data[i].dist[c] - pMean;
        const resid = data[i].dist[c] - pred;
        totalSS += ws[i] * diff * diff;
        residSS += ws[i] * resid * resid;
      }
    }
    ...
    const r2 = totalSS > 1e-10 ? 1 - residSS / totalSS : 0;
    return { dist: result, nRounds: data.length, r2: Math.max(0, r2) };
  }
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      if (fineReg && coarseReg && key !== ck) {
        ...
        regR2 = alpha * fineReg.r2 + (1 - alpha) * coarseReg.r2;
      } else if (fineReg) {
      ...
      // Adaptive blend: more regression when R² is high
      const regBlend = regR2 > 0.1 ? 0.6 + 0.3 * Math.min(1, regR2 / 0.5) : 0.5;
