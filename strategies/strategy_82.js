...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  var settSigma = config.settSigma || 8;
  ...
  // Infer settlement count per training round from bucket data
  var roundSettCounts = {};
  for (var rn of Object.keys(perRoundBuckets)) {
    var b = perRoundBuckets[rn];
    var sc = 0;
    for (var key in b) {
      if (key[0] === 'S') sc += b[key].count;
    }
  ...
  // LOESS regression (growth-only weights, same as strategy_76)
  var linCache = {};
  ...
  // Gaussian mean model with MULTI-FEATURE weights (growth + settlement count)
  var roundWeights = {};
  var tw = 0;
  for (var rn of allRounds) {
    var gDiff = (growthRates[String(rn)] || 0.15) - targetGrowth;
    var sDiff = (roundSettCounts[rn] || 35) - testSettCount;
    var gwt = Math.exp(-gDiff * gDiff / (2 * sigma * sigma));
    var swt = Math.exp(-sDiff * sDiff / (2 * settSigma * settSigma));
    roundWeights[rn] = gwt * swt;
    tw += roundWeights[rn];
  }
