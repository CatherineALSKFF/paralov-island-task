...
// Enhanced feature key: adds Chebyshev distance-to-nearest-settlement (a/b)
// for cells with 1-2 nearby settlements, matching the 43-key bucket structure
...
function predict(initGrid, settlements, perRoundBuckets, growthRates, testRound, config) {
  ...
  // Gaussian round weighting with tight sigma for precise growth matching
  ...
  // Top-K model: uniform over K=2 closest rounds
  const K = config.K || 2;
  ...
  // Progressive shrinkage key chain: P1ca -> P1c -> P1 -> P
  ...
  // Progressive Bayesian shrinkage toward coarser features
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      // Adaptive floor: higher for uncertain cells, lower for confident ones
