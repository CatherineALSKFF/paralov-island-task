#!/usr/bin/env node
// R8 FINAL BEST — Multi-level VP + all cells (ultra-conservative 1-obs)
...
async function main() {
  ...
  for (const level of ['d0', 'd1', 'd2']) {
    ...
    for (const [k, vp] of Object.entries(vm)) {
      ...
      if (bm) {
        ...
        model[k] = { n: bm.n + vp.n, a: post.map(v => v / tot) };
      } else if (level === 'd0') {
        ...
        if (cm) {
          ...
          const tot = post.reduce((a, b) => a + b, 0);
          model[k] = { n: vp.n + cw, a: post.map(v => v / tot) };
        } else {
          const tot = vp.n + C * 0.1;
          model[k] = { n: vp.n, a: Array.from(vp.counts).map(v => (v + 0.1) / tot) };
        }
      }
    }
  }
  ...
  const TEMP = 1.1;
  console.log(`Submitting: temp=${TEMP}, ALL cells, ultra-conservative 1-obs`);
  ...
  for (let si = 0; si < SEEDS; si++) {
    ...
    for (const [key, cell] of Object.entries(cellModels[si])) {
      ...
      // Ultra-conservative for 1-obs, aggressive for 5+
      let pw;
      if (cell.n >= 5) { pw = 2; stats.n5++; }
      else if (cell.n >= 3) { pw = 4; stats.n3++; }
      else if (cell.n >= 2) { pw = 7; stats.n2++; }
      else { pw = 15; stats.n1++; } // 1 obs: 1/16 = 6.25% influence
    ...
    console.log(`Seed ${si}: ${res.ok ? 'OK' : 'FAIL'} [1obs:${stats.n1} 2obs:${stats.n2} 3obs:${stats.n3} 5+obs:${stats.n5} = ${total}]`);
    await sleep(600);
  }
  
  console.log('\n=== FINAL BEST SUBMITTED ===');
  console.log('Multi-level VP (D0:cw=20, D1:cw=15, D2:cw=10) + temp=1.1');
  console.log('Per-cell ALL: 1obs→pw=15, 2obs→pw=7, 3obs→pw=4, 5+obs→pw=2');
}
