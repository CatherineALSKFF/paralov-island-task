#!/usr/bin/env node
// R8 SAFE — Feature model + viewport D0 + per-cell ONLY for >=3 obs (safe)
// Theory: 1 observation per-cell adds noise. Feature-level is more robust.
...
async function main() {
  console.log('=== R8 SAFE SUBMISSION (no risky per-cell) ===');
  const I = {}, G = {}, R = {}, TR = [];
  ...
  // Multi-level cross-round (GTW=20)
  const model = {};
  ...
  // D0 viewport (combined all seeds)
  const vpM = {};
  for (const obs of vpObs) { const si = obs.si !== undefined ? obs.si : 0;
    for (let dy = 0; dy < obs.grid.length; dy++) for (let dx = 0; dx < obs.grid[0].length; dx++) {
      ...
      if (!vpM[k]) vpM[k] = { n: 0, counts: new Float64Array(C) }; vpM[k].n++; vpM[k].counts[fc]++; } }
  console.log('VP D0:', Object.keys(vpM).length, 'keys');

  // Fuse (cw=20)
  const CW = 20;
  for (const [k, vm] of Object.entries(vpM)) {
    const bm = model[k];
    if (bm) {
  ...
  // Per-cell ONLY for cells with >=3 obs (safe, well-validated)
  const obsBySeed = {};
  for (const obs of vpObs) { const si = obs.si !== undefined ? obs.si : 0;
  ...
  for (let si = 0; si < SEEDS; si++) {
    ...
    cellModels[si] = cells;
    console.log(`Seed ${si}: ${Object.values(cells).filter(c => c.n >= 3).length} cells with >=3 obs`);
  }

  // Submit with temp=1.1, per-cell pw=5 ONLY for >=3 obs
  console.log('\nSubmitting: temp=1.1, per-cell pw=5 (>=3 obs only, safe)');
  for (let si = 0; si < SEEDS; si++) {
    let pred = predict(inits[si], model, 1.1);
    // Apply per-cell ONLY for cells with >=3 observations
    for (const [key, cell] of Object.entries(cellModels[si])) {
      if (cell.n < 3) continue; // SKIP noisy 1-2 obs cells
      const [y, x] = key.split(',').map(Number);
      if (inits[si][y][x] === 10 || inits[si][y][x] === 5) continue;
      const prior = pred[y][x], posterior = new Array(C); let total = 0;
      for (let c = 0; c < C; c++) { posterior[c] = prior[c] * 5 + cell.counts[c]; total += posterior[c]; }
    ...
    console.log(`Seed ${si}: ${res.ok ? 'ACCEPTED' : 'FAILED'}`);
    await sleep(600);
  }
  console.log('\n=== SAFE R8 SUBMITTED ===');
  console.log('Using feature-level model + per-cell ONLY for reliable (>=3 obs) cells');
}
